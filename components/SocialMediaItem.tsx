...
export default function SocialMediaItem({ contact }: ContactViewProps) {
  ...
  const FinalIcon = () => {
    switch (name) {
      case 'Github':
        return <BxlGithub className="size-11" />
      case 'Gmail':
        return <BxlGmail className="size-11" />
      case 'Linkedin':
        return <BxlLinkedin className="size-11" />
      case 'Whatsapp':
        return <BxlWhatsapp className="size-11" />
      case 'Email':
        return <FlowbiteEnvelopeOutline className="size-11" />
      case 'wwww':
        return <IconoirWww className="size-11" />
      default:
        return <IconoirWww className="size-11" />
    }
  }
  ...
  if (show) {
    return (
      <a
        className="mb-5 flex items-center gap-x-5 hover:text-primary"
        target="_blank"
        href={finalLink}
      >
        <FinalIcon />
